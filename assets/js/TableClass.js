...
let defaultFields = ['latitude', 'longitude', 'name', 'description', 'submission_date', 'author'];
class TableUI {
    constructor(HTMLParentId, desiredFields = defaultFields, currentUserOnly = false) {
        this.desiredFields = desiredFields;
        this.currentUserOnly = currentUserOnly;
        this.sortingOrder = { by: null, asc: null };
        this.data = null;
        this.tableDiv = document.getElementById(HTMLParentId);
    ...
    async getData() {
        let queryUrl = this.getCorrectQueryUrl();
    ...
    getCorrectQueryUrl() {
        if (this.desiredFields === defaultFields && !this.currentUserOnly)
            return '/api/getAllSubmissions'
        else if (this.desiredFields === defaultFields && this.currentUserOnly) {
            return '/api/getSubmissions?currentUserOnly=true'
        } else if (this.desiredFields !== defaultFields && !this.currentUserOnly) {
            return '/api/getSubmissions?fields=' + this.createFieldsQueryString();
        } else {
            return '/api/getSubmissions?currentUserOnly=true&fields=' + this.createFieldsQueryString();
        }
    }
    createFieldsQueryString() {
        let fieldsJSON = JSON.stringify(this.desiredFields);
        return encodeURIComponent(fieldsJSON);
    }
